...
def _check_phases(phases, n_components):
    ...
    if (any(np.less(phases, -360.)) or
        ...
        raise ValueError("The parameter 'phases' should be in the range "
                         "[-360, 360], but got min value %.5f, max value %.5f"
...
class PhaseConstrainedGaussianMixture(PhaseConstrainedBaseMixture):
    """Phase First Gaussian Mixture.

    This class implements the Phase First Gaussian mixture fit.
    While fitting to a dataset in polar coordinates, the phase dimension is fit to
